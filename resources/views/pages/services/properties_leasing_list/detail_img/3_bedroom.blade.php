...
@section('content')
    @php
        $roomSlug = $roomSlug ?? 'daily-weekly-rentals-3-bedroom';
        $heroData = \App\Models\HeroSection::where('page', $roomSlug)->first();
        $heroTaglineHtml = $heroData?->tagline_html ?: ($heroData?->tagline_box1 ?: 'Daily &amp; Weekly Rentals');
        $heroHeadline = $heroData?->headline ?: '3-Bedroom Suite';
        $heroSubtext = (!empty($heroData?->bullets) && is_array($heroData->bullets) && count($heroData->bullets) > 0) ? $heroData->bullets[0] : "Expansive living spaces designed for large families, executive relocations, and luxury comfort.";
    ...
    @php
        ...
        $showcaseData = \App\Models\ServiceMaximizeSection::where('page', $roomSlug)->first();
        $showcaseTitle = $showcaseData?->title ?: 'Flexible 3-Bedroom Suite Rentals at Wealth Mansion';
        $showcaseParagraphs = (!empty($showcaseData?->paragraphs) && is_array($showcaseData->paragraphs) && count($showcaseData->paragraphs) > 0) 
            ...
            : ['Expansive luxury suites with premium master bedroom, high-end living quarters, and top-tier hospitality services for executive relocations.'];

        // Fetch dynamic gallery photos from ProjectGallery
        $dbGallery = \App\Models\ProjectGallery::where('page', $roomSlug)
        ...
        if ($dbGallery->count() > 0) {
            foreach ($dbGallery as $g) {
                ...
                $discoverItems[] = [
                    'image' => $imgUrl,
                    'title' => $g->title ?: ($g->alt_text ?: '3-Bedroom Suite view')
                ];
            }
        } else {
            $rawImages = [
            ...
            foreach ($rawImages as $idx => $raw) {
                $discoverItems[] = [
                    'image' => $raw,
                    'title' => '3-Bedroom Suite view ' . ($idx + 1)
                ];
            }
        }
    @endphp

    {{-- Flexible Condominium Rentals Section --}}
    <section class="relative z-[30] bg-transparent px-6 sm:px-10 lg:px-14 pb-16 lg:pb-24 -mt-[90px] sm:-mt-[130px] lg:-mt-[200px] overflow-x-clip">
        <div class="max-w-[1400px] mx-auto">
            ...
            <div class="flex flex-col lg:flex-row items-start justify-between gap-6 lg:gap-10 h-auto lg:h-[600px]">
                ...
                <div id="bedroom3-carousel-height-wrapper"
                    ...
                    {{-- Image Track --}}
                    <div id="bedroom3-carousel-track"
                        ...
                        @foreach ($discoverItems as $index => $item)
                            <button type="button"
                                ...
                                class="bedroom3-carousel-item relative overflow-hidden rounded-none shadow-md transform-gpu
                ...
                <div class="flex flex-col items-end lg:items-start shrink-0 w-full lg:w-auto lg:max-w-[280px] xl:max-w-[340px] order-1 lg:order-2 pt-[110px] sm:pt-[140px] lg:pt-[220px] xl:pt-[230px]" data-scroll-reveal="left">
                    ...
                    <div class="flex items-center gap-3 mt-4 sm:mt-6 lg:mt-0 mb-6 sm:mb-8 lg:mb-10 self-end lg:self-start">
                        <button id="bedroom3-carousel-prev" type="button" aria-label="Previous image"
                        ...
                        <button id="bedroom3-carousel-next" type="button" aria-label="Next image"
                            class="w-11 h-11 sm:w-12 sm:h-12 rounded-full border-[1.5px] border-[#2A5A8A] text-[#2A5A8A] bg-white flex items-center justify-center cursor-pointer
                            transition-all duration-300 hover:bg-[#2A5A8A] hover:text-white hover:scale-105 shadow-sm
                            focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2A5A8A] focus-visible:ring-offset-2
                            disabled:cursor-not-allowed">
    ...
    <div id="bedroom3-lightbox-modal"
        ...
        {{-- Close Button --}}
        <button id="bedroom3-lightbox-close" type="button" aria-label="Close preview"
        ...
        {{-- Previous Button --}}
        <button id="bedroom3-lightbox-prev" type="button" aria-label="Previous image"
        ...
        {{-- Next Button --}}
        <button id="bedroom3-lightbox-next" type="button" aria-label="Next image"
        ...
        {{-- Gallery Stage --}}
        <div class="relative flex items-center justify-center gap-3 sm:gap-6 lg:gap-8 w-full max-w-[96vw] max-h-[85vh] px-9 sm:px-14 md:px-0 select-none" id="bedroom3-lightbox-content">
            
            {{-- Left Preview Image --}}
            <div id="bedroom3-lightbox-prev-card"
                ...
                <img id="bedroom3-lightbox-prev-img" src="" alt="Previous View Preview"
            ...
            <div class="relative flex flex-col items-center justify-center shrink-0 w-full max-w-full md:max-w-[62vw] lg:max-w-[58vw] max-h-[82vh] z-20">
                <img id="bedroom3-lightbox-img" src="" alt="Wealth Mansion 3-Bedroom Full View"
                ...
                <div class="mt-4 flex items-center justify-between w-full px-2 text-white/90 text-sm font-medium">
                    <span id="bedroom3-lightbox-title">Wealth Mansion 3-Bedroom Suite</span>
                    <span id="bedroom3-lightbox-counter" class="bg-white/15 px-3 py-1 rounded-full text-xs font-semibold text-white"></span>
                </div>
            </div>

            {{-- Right Preview Image --}}
            <div id="bedroom3-lightbox-next-card"
                class="hidden md:flex flex-col items-center justify-center shrink-0 w-[14vw] lg:w-[16vw] max-h-[50vh] opacity-35 hover:opacity-80 transition-all duration-300 transform scale-80 cursor-pointer pointer-events-auto group">
                <img id="bedroom3-lightbox-next-img" src="" alt="Next View Preview"
                    class="w-full max-h-[45vh] object-contain rounded shadow-lg transition-all duration-300 group-hover:scale-105 filter brightness-90">
            </div>
    ...
    @once
        <script>
            (function() {
                function initBedroom3Carousel() {
                    const track = document.getElementById("bedroom3-carousel-track");
                    if (!track) return;

                    const items = Array.from(track.querySelectorAll(".bedroom3-carousel-item"));
                    if (!items.length) return;

                    const prevBtn = document.getElementById("bedroom3-carousel-prev");
                    const nextBtn = document.getElementById("bedroom3-carousel-next");
                    ...
                    // Lightbox Elements
                    const lightbox = document.getElementById("bedroom3-lightbox-modal");
                    const lightboxImg = document.getElementById("bedroom3-lightbox-img");
                    const lightboxPrevImg = document.getElementById("bedroom3-lightbox-prev-img");
                    const lightboxNextImg = document.getElementById("bedroom3-lightbox-next-img");
                    const lightboxPrevCard = document.getElementById("bedroom3-lightbox-prev-card");
                    const lightboxNextCard = document.getElementById("bedroom3-lightbox-next-card");
                    const lightboxCounter = document.getElementById("bedroom3-lightbox-counter");
                    const lightboxTitle = document.getElementById("bedroom3-lightbox-title");
                    const lightboxClose = document.getElementById("bedroom3-lightbox-close");
                    const lightboxPrev = document.getElementById("bedroom3-lightbox-prev");
                    const lightboxNext = document.getElementById("bedroom3-lightbox-next");
                    const lightboxContent = document.getElementById("bedroom3-lightbox-content");
                ...
                if (document.readyState === "loading") {
                    document.addEventListener("DOMContentLoaded", initBedroom3Carousel);
                } else {
                    initBedroom3Carousel();
                }
            })();
        </script>
    @endonce
