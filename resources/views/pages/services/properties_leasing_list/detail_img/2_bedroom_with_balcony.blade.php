...
@section('content')
    @php
        $roomSlug = $roomSlug ?? 'daily-weekly-rentals-2-bedroom-with-balcony';
        ...
        $heroHeadline = $heroData?->headline ?: '2-Bedroom Balcony';
        $heroSubtext = (!empty($heroData?->bullets) && is_array($heroData->bullets) && count($heroData->bullets) > 0) ? $heroData->bullets[0] : "More space for families, colleagues, or guests requiring an additional bedroom.";
    ...
    @php
        ...
        $showcaseTitle = $showcaseData?->title ?: 'Flexible 2-Bedroom Balcony Rentals at Wealth Mansion';
        $showcaseParagraphs = (!empty($showcaseData?->paragraphs) && is_array($showcaseData->paragraphs) && count($showcaseData->paragraphs) > 0) 
            ...
            : ['Generous 2-bedroom space with private panoramic balcony, ideal for small families, visiting teams, and extended residential stays.'];
        ...
        if ($dbGallery->count() > 0) {
            foreach ($dbGallery as $g) {
                ...
                $discoverItems[] = [
                    'image' => $imgUrl,
                    'title' => $g->title ?: ($g->alt_text ?: '2-Bedroom Balcony view')
                ];
            }
        } else {
            $rawImages = [
            ...
            foreach ($rawImages as $idx => $raw) {
                $discoverItems[] = [
                    'image' => $raw,
                    'title' => '2-Bedroom Balcony view ' . ($idx + 1)
                ];
            }
        }
    @endphp
                    ...
                    <div id="bedroom2-carousel-track"
                        ...
                        @foreach ($discoverItems as $index => $item)
                            <button type="button"
                                ...
                                class="bedroom2-carousel-item relative overflow-hidden rounded-none shadow-md transform-gpu
    ...
    <div id="bedroom2-lightbox-modal"
        ...
        {{-- Close Button --}}
        <button id="bedroom2-lightbox-close" type="button" aria-label="Close preview"
        ...
        {{-- Previous Button --}}
        <button id="bedroom2-lightbox-prev" type="button" aria-label="Previous image"
        ...
        {{-- Next Button --}}
        <button id="bedroom2-lightbox-next" type="button" aria-label="Next image"
        ...
        {{-- Gallery Stage --}}
        <div class="relative flex items-center justify-center gap-3 sm:gap-6 lg:gap-8 w-full max-w-[96vw] max-h-[85vh] px-9 sm:px-14 md:px-0 select-none" id="bedroom2-lightbox-content">
            
            {{-- Left Preview Image --}}
            <div id="bedroom2-lightbox-prev-card"
                ...
                <img id="bedroom2-lightbox-prev-img" src="" alt="Previous View Preview"
            ...
            <div class="relative flex flex-col items-center justify-center shrink-0 w-full max-w-full md:max-w-[62vw] lg:max-w-[58vw] max-h-[82vh] z-20">
                <img id="bedroom2-lightbox-img" src="" alt="Wealth Mansion 2-Bedroom Full View"
                ...
                <div class="mt-4 flex items-center justify-between w-full px-2 text-white/90 text-sm font-medium">
                    <span id="bedroom2-lightbox-title">Wealth Mansion 2-Bedroom Balcony</span>
                    <span id="bedroom2-lightbox-counter" class="bg-white/15 px-3 py-1 rounded-full text-xs font-semibold text-white"></span>
                </div>
            </div>

            {{-- Right Preview Image --}}
            <div id="bedroom2-lightbox-next-card"
                class="hidden md:flex flex-col items-center justify-center shrink-0 w-[14vw] lg:w-[16vw] max-h-[50vh] opacity-35 hover:opacity-80 transition-all duration-300 transform scale-80 cursor-pointer pointer-events-auto group">
                <img id="bedroom2-lightbox-next-img" src="" alt="Next View Preview"
                    class="w-full max-h-[45vh] object-contain rounded shadow-lg transition-all duration-300 group-hover:scale-105 filter brightness-90">
            </div>
        ...
        <script>
            (function() {
                function initBedroom2Carousel() {
                    ...
                    // Lightbox Elements
                    const lightbox = document.getElementById("bedroom2-lightbox-modal");
                    const lightboxImg = document.getElementById("bedroom2-lightbox-img");
                    const lightboxPrevImg = document.getElementById("bedroom2-lightbox-prev-img");
                    const lightboxNextImg = document.getElementById("bedroom2-lightbox-next-img");
                    const lightboxPrevCard = document.getElementById("bedroom2-lightbox-prev-card");
                    const lightboxNextCard = document.getElementById("bedroom2-lightbox-next-card");
                    const lightboxCounter = document.getElementById("bedroom2-lightbox-counter");
                    const lightboxTitle = document.getElementById("bedroom2-lightbox-title");
                    const lightboxClose = document.getElementById("bedroom2-lightbox-close");
                    const lightboxPrev = document.getElementById("bedroom2-lightbox-prev");
                    const lightboxNext = document.getElementById("bedroom2-lightbox-next");
                    const lightboxContent = document.getElementById("bedroom2-lightbox-content");
